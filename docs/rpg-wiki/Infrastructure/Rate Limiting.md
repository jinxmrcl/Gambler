...
- **Per-channel** (3 requests per 5s per channel) — `limited_edit()`, since Discord rate-limits
  message edits per-channel. Used by [[../Casino Games/Slots|Slots]]' spin animation and every
...
- **Global** (45 requests/second bot-wide, under Discord's hard 50/s ceiling) — every
  ...
  sends (currently used by [[../Economy/Payday|Payday]]'s announcements), so total bot-wide
