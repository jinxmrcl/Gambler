...
- `/idle <dungeon> [minutes=30] [auto=False]` — auto-farms that dungeon's regular monsters and its boss in the background, then delivers one final summary — no per-fight spam. The summary is always posted as a fresh, pinging message in the channel `/idle` was run in only (never an edit — Discord invalidates the command's reply token after 15 minutes, so editing the original message can't be relied on for a run this long), and includes fight/boss win counts, a per-monster kill breakdown (`Name (xN)`), total gold and XP earned, any level-up, and any loot (`Item (xN)`). Progress and the remaining deadline are saved to the database every farming tick, so a hot-reload or restart pauses the run rather than losing it — it silently picks back up (still counting down toward the original end time) the next time the bot comes online, with no interrupted/partial message shown for that.
  - By default (`auto` off) it runs for a fixed `minutes` (up to 120) and stops on its own.
  - With `auto: True` it ignores `minutes` and instead runs indefinitely in 60-minute checkpoints: every hour it posts a non-pinging checkpoint summary of that hour's progress, resets the counters, and keeps going — forever, until you run `/idle` again for any dungeon, which stops it and delivers a final pinging summary of whatever progress had accumulated since the last checkpoint. This also survives hot-reloads/restarts (the auto flag and in-progress checkpoint stats are persisted, not just the deadline).
  - A separate tracker message in a configurable channel (`IDLE_ANNOUNCE_CHANNEL_ID`) lists everyone currently idle farming, showing auto sessions as "next checkpoint in Xm" instead of a countdown to an end — it refreshes every minute plus whenever someone starts or finishes (a burst of several events, like many sessions resuming at once after a restart, is debounced into a single edit rather than one per event), and reposts itself (rather than editing forever) once it's over 12 hours old
