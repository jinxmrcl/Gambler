...
class Hustle(commands.Cog):
    ...
    async def _claim_cooldown(self, ctx: commands.Context, db, action: str, seconds: int) -> bool:
        now = datetime.datetime.utcnow()
        ok = await db.try_consume_cooldown(
            ctx.author.id, action, datetime.timedelta(seconds=seconds), now
        )
        if not ok:
            until = await db.get_cooldown(ctx.author.id, action)
    ...
    async def work(self, ctx: commands.Context):
        ...
        if not await self._claim_cooldown(ctx, db, "work", WORK_COOLDOWN):
            return
        amount = random.randint(50, 200)
        new_balance = await db.update_balance(ctx.author.id, amount)
    ...
    async def crime(self, ctx: commands.Context):
        ...
        db = await self.bot.db.get(ctx.guild.id)
        await db.ensure_user(ctx.author.id, self.bot.starting_balance)
        if not await self._claim_cooldown(ctx, db, "crime", CRIME_COOLDOWN):
            return
        ...
        if random.random() < 0.7:
            ...
            new_balance = await db.update_balance(ctx.author.id, -fine)
    ...
    async def slut(self, ctx: commands.Context):
        ...
        db = await self.bot.db.get(ctx.guild.id)
        await db.ensure_user(ctx.author.id, self.bot.starting_balance)
        if not await self._claim_cooldown(ctx, db, "slut", SLUT_COOLDOWN):
            return
        ...
        if random.random() < 0.65:
            ...
            new_balance = await db.update_balance(ctx.author.id, -loss)
    ...
    async def rob(self, ctx: commands.Context, user: discord.User):
        ...
        protected_until = await db.get_protected_until(user.id)
        ...
        if success:
            ...
            try:
                ...
                stolen = await db.get_balance(user.id)
                await db.update_balance(user.id, -stolen)
            new_balance = await db.update_balance(ctx.author.id, stolen)
            await db.record_robbed(user.id)
