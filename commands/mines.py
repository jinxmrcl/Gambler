...
def multiplier_for(safe_revealed: int, mines: int) -> float:
    if safe_revealed == 0:
        return 1.0
...
class MinesView(ui.LayoutView):
    def __init__(self, cog: "Mines", ctx: commands.Context, bet: int, mines: int):
    ...
    def current_multiplier(self) -> float:
        return multiplier_for(self.revealed, self.mines)
    ...
    async def reveal(self, interaction: discord.Interaction, button: TileButton):
        ...
        if self.revealed == TOTAL_TILES - self.mines:
...
class Mines(commands.Cog):
    ...
    async def mines(self, ctx: commands.Context, bet: str, mines: app_commands.Range[int, 1, TOTAL_TILES - 1] = 3):
        ...
        view = MinesView(self, ctx, amount, mines)
